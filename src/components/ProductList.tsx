...
export default async function ProductList({
	...
	const paginationData = [
		{
			...
			totalCount: sortedAndTyped.length,
			offset: 0,
		},
	]

	if (searchParams?.page !== undefined && searchParams.page !== '') {
		paginationData[0].offset = parseInt(searchParams.page) * paginationData[0].limit
	}
	...
	return (
		<section className="flex gap-x-8 gap-y-16 flex-wrap mt-12">
			...
			{searchParams?.cat !== undefined && (
				<Pagination
					currentPage={paginationData[0].offset / paginationData[0].limit}
					hasPrev={paginationData[0].offset > 0}
					hasNext={
						paginationData[0].offset + paginationData[0].limit <
						paginationData[0].totalCount
					}
					maxPages={Math.ceil(paginationData[0].totalCount / paginationData[0].limit)}
				/>
			)}
		</section>
	)
